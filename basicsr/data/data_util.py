...
def paired_DP_paths_from_folder(folders, keys, filename_tmpl):
    ...
    lrL_files = natsorted(glob(inputL_folder+'/*.png'))
    lrR_files = natsorted(glob(inputR_folder+'/*.png'))
    lrC_files = natsorted(glob(inputC_folder+'/*.png'))
    hrC_files = natsorted(glob(gt_folder+'/*.png'))
    paths = [dict([(f'{inputL_key}_path', inputL_path),
                   (f'{inputR_key}_path', inputR_path),
                   (f'{inputC_key}_path', inputC_path),
                   (f'{gt_key}_path', gt_path)]) for inputL_path, inputR_path, inputC_path, gt_path in
             zip(lrL_files, lrR_files, lrC_files, hrC_files)]
    ...
    #     inputL_name = f'{filename_tmpl.format(basename)}_L{ext_input}'
    ...
    #     inputR_name = f'{filename_tmpl.format(basename)}_R{ext_input}'
